...
export default class JobsResume extends BaseCommand {
  static description = 'resume job on Cron To Go\nRead more about this feature at https://devcenter.heroku.com/articles/crontogo'
  ...
  async run(): Promise<void> {
    const {args, flags} = this.parse(JobsResume)
    ...
    CliUx.ux.action.start(`Resuming job ${color.cyan(job)}`)
    try {
      await this.api.put<Job>(
        `/organizations/${authInfo.organizationId}/jobs/${job}/resume`,
        {
          ...this.api.defaults,
          raw: true,
        },
