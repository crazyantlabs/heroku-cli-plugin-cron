...
export const COLORS: Array<(s: string) => string> = [
  (s: string): string => c.yellow(s),
  (s: string): string => c.green(s),
  (s: string): string => c.cyan(s),
  (s: string): string => c.magenta(s),
  (s: string): string => c.blue(s),
  (s: string): string => c.bold.green(s),
  (s: string): string => c.bold.cyan(s),
  (s: string): string => c.bold.magenta(s),
  (s: string): string => c.bold.yellow(s),
  (s: string): string => c.bold.blue(s),
]
...
const ms = (s: string) => {
  ...
  if (ms < 10_000) return c.yellowBright(s)
  return c.red(s)
}
...
function colorizeRun(body: string) {
  try {
    if (/^Stopping all processes with SIGTERM$/.test(body)) return c.red(body)
...
function colorizeWeb(body: string) {
  try {
    if (/^Unidling$/.test(body)) return c.yellow(body)
    if (/^Restarting$/.test(body)) return c.yellow(body)
    if (/^Stopping all processes with SIGTERM$/.test(body)) return c.red(body)
    const starting = body.match(/^(Starting process with command )(`.+`)(by user )?(.*)?$/)
    if (starting) {
      return [
    ...
    const apache = body.match(/^((?:\d+\.){3}\d+ -[^-]*- \[[^\]]+] ")(\w+)( )([^ ]+)( HT{2}P\/\d+\.\d+" )(\d+)( .+$)/)
    if (apache) {
      const [, ...tokens] = apache
      return [
        other(tokens[0]),
...
function colorizeAPI(body: string) {
  if (/^Build succeeded$/.test(body)) return c.greenBright(body)
  if (body.startsWith('Build failed')) return c.red(body)
